...
func GetSqlOfTableInfoForPostgres(table types.TableInfo, tblMap *types.TableMapping) []string {
	ret := []string{}
	// create table
	sql := "CREATE TABLE IF NOT EXISTS \"" + table.TableName + "\" ("
	colsScript := []string{}
	scripAlterAddCols := []string{}
	colsInfb := map[string]string{}
	if tblMap != nil {
		if tblFormMap, ok := (*tblMap)[strings.ToLower(table.TableName)]; ok {
			colsInfb = tblFormMap.ColInfos
		}
		...
		if dbType, ok := mapGoTypeToPosgresType[ft]; ok {
			if col.IsPrimary {
				...
				if col.AllowNull {
					...
					if _, ok := colsInfb[strings.ToLower(col.Name)]; !ok {
	...
	var tblFormMap *types.DbTbaleInfo = nil
	// check if table is ready in map
	if tblMap != nil {
		tblFormMap_, ok := (*tblMap)[strings.ToLower(table.TableName)]
		if !ok {
			sql += strings.Join(colsScript, ",") + ")"
			ret = append(ret, sql)
		} else {
			tblFormMap = &tblFormMap_
		}
	}
	...
	for _, rel := range table.Relationship {
		if ret != nil {
			sqlOfToTable := GetSqlOfTableInfoForPostgres(rel.ToTable, tblMap)
